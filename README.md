[![Typing SVG](https://readme-typing-svg.demolab.com/?lines=Hi+there,+I'm+Hossein+👋;)](https://git.io/typing-svg)

Enthusiastic in Artificial Intelligence 🤖 and Data Analysis 📊 fields. Here is my digital workspace where I share my learning journey (and it's all about the journey and not the destination 🍃), projects, and professional growth.

---

### 📖 About Me
After a decade of academic and professional experience in other fields, I finally discovered my true calling: **Machine Learning and Artificial Intelligence.** 

I am currently on an exciting journey to master these technologies. My daily routine involves diving deep into code, analyzing scientific papers, and exploring advanced AI architectures. I am currently pursuing specialized courses at [Sharif University of Technology/Your Institution], and I am passionate about contributing to innovative projects in a remote-first environment.

---

### 🛠 Tech Stack

<p align="center">
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/python/python-original.svg" alt="python" width="45" height="45"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/pytorch/pytorch-original.svg" alt="pytorch" width="45" height="45"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/tensorflow/tensorflow-original.svg" alt="tensorflow" width="45" height="45"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/git/git-original.svg" alt="git" width="45" height="45"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/vscode/vscode-original.svg" alt="vscode" width="45" height="45"/>
  <img src = "https://raw.githubusercontent.com/devicons/devicon/master/icons/scikitlearn/scikitlearn-original.svg" alt="sklearn" width="45" height="45"/>
  <img src = "https://github.com/devicons/devicon/blob/master/icons/jupyter/jupyter-original-wordmark.svg" alt = "jupyter" width="45" height="45"/>
  <img src = "https://github.com/devicons/devicon/blob/master/icons/numpy/numpy-line-wordmark.svg" alt = "numpy" width="60" height="60"/>
  <img src = "https://github.com/devicons/devicon/blob/master/icons/pandas/pandas-original-wordmark.svg" alt = "pandas" width="45" height="45"/>
  <img src = "https://github.com/devicons/devicon/blob/master/icons/matplotlib/matplotlib-original-wordmark.svg" alt = "matplotlib" width="60" height="60"/>
  <img src = "https://github.com/devicons/devicon/blob/master/icons/plotly/plotly-original-wordmark.svg" alt = "plotly" width="60" height="60"/>
  <img src="https://cdn.simpleicons.org/obsidian/7C3AED" alt="Obsidian" width="45" height="45"/> </p>
</p>

- 🐍 **Languages:** Python, SQL
- 🧠 **ML/DL Frameworks:** PyTorch, TensorFlow, Scikit-Learn, Pandas, NumPy
- ⚙️ **Engineering Tools:** Git, VS Code, Docker, Jupyter Notebook
- 📊 **Visualization Tools:** matplotlib, plotly
- ✍🏽 **Noting and Summarization:** Obsidian

---

### 🚀 What I'm working on right now
- **Deep Diving:** Exploring advanced concepts in Deep Learning, NLP, and Generative Models.
- **Skill Building:** Continuously refining my programming workflows and software engineering best practices.
- **Goal:** Aspiring to become a well-rounded ML & AI Engineer who genuinely wants to speeding up things and making them smarter! (because some time the old fashion ways works better so no need for revolution 😉)

---

### 📬 Let's Connect
✉️ **Email:** [hoseinrezazadeh51@gmail.com](mailto:hoseinrezazadeh51@gmail.com)

---
### 💡 My Philosophy
> *Learning slowly, growing steady, and refusing to become another brick in the wall.* (pink floyd)
